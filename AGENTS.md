...
| **FormRequest** | `FormRequestInterface`, validation, factory, resolver | WIP |
| **Casting** | Valinor `Caster`, `TypedRouteParameterArgumentResolver` | ✅ |
...
| (майбутні) | Session, Database, Validation, … | з `storage/src/` |
...
**FormRequest** (WIP) — ще flat layout; при переносі → той самий пакетний шаблон + `php-concept/extension-form-request`.
...
### FormRequest extension (WIP, зламаний)

`src/Extensions/FormRequest/FormRequestResolver.php` — чернетка з помилками:
- неправильні imports (`Concept\App\Http\Routing\Resolvers\*`)
- немає `$className`, немає `getContainer()`
- namespace: `Concept\Extensions\FormRequest`

`src/Extensions/FormRequest/FormRequest.php` — **старий** код з `Concept\Core\Http\Requests`, потребує рефакторингу під extension.

**Autoload:** `Concept\Extensions\` → `src/Extensions/` у `composer-dev.json`.
...
- [x] **Casting extension**: Caster (Valinor), TypedRouteParameterArgumentResolver
- [x] Autoload `Concept\Extensions\` у skeleton
...
1. **Extension FormRequest**: виправити contracts, resolver, factory, provider
2. **Skeleton**: зареєструвати FormRequest provider + resolver у `bootstrap/providers.php`
3. **Extension Casting**: перенести Caster з `storage/src/Services/Caster/`, typed route resolver
4. **Extension View**: ViewResponseFactory після View engine
5. **Http extension** (далі): middleware (`ParseJsonBody`, `ForceJsonResponse`, …)
6. **Extension Console**: `RouteListCommand` та інші CLI commands
7. Поступово переносити інші можливості з `storage/src/` у extensions
8. **Extension Config** (фінал): читання `storage/config/`, прокидання параметрів у glue
...
*Останнє оновлення: 2026-06-24 — Http extension; enriched request без container refresh.*
