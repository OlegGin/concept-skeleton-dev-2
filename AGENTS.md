...
- **Перенос з `storage/src/`** — зберігати поведінку та структуру класу (поля, методи, порядок логіки); не переносити внутрішню логіку в config/glue і не змінювати публічний API без явної згоди
